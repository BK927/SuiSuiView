...
pub(super) struct Anime4kMRenderer {
    conv_pipeline: wgpu::RenderPipeline,
...
impl Anime4kMRenderer {
    pub(super) fn new(device: &wgpu::Device) -> Self {
        ...
        let conv_bind_group_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("suisuiview-realtime-anime4k-m-conv-bind-group-layout"),
                entries: &[
                    texture_entry(0),
                    wgpu::BindGroupLayoutEntry {
                        binding: 1,
        ...
        let combine_bind_group_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("suisuiview-realtime-anime4k-m-combine-bind-group-layout"),
                entries: &[
                    texture_entry(0),
                    texture_entry(1),
                    texture_entry(2),
        ...
        let final_bind_group_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("suisuiview-realtime-anime4k-m-final-bind-group-layout"),
                entries: &[texture_entry(0), texture_entry(1)],
            });
        let conv_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-conv-pipeline-layout"),
            bind_group_layouts: &[&conv_bind_group_layout],
            push_constant_ranges: &[],
        });
        let combine_pipeline_layout =
            device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("suisuiview-realtime-anime4k-m-combine-pipeline-layout"),
                bind_group_layouts: &[&combine_bind_group_layout],
                push_constant_ranges: &[],
            });
        let final_pipeline_layout =
            device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("suisuiview-realtime-anime4k-m-final-pipeline-layout"),
                bind_group_layouts: &[&final_bind_group_layout],
                push_constant_ranges: &[],
            });
        let conv_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-conv-pipeline"),
            layout: Some(&conv_pipeline_layout),
            vertex: vertex_state(&shader),
            fragment: Some(wgpu::FragmentState {
                module: &shader,
        ...
        let combine_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-combine-pipeline"),
        ...
        let final_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-final-pipeline"),
            layout: Some(&final_pipeline_layout),
            vertex: vertex_state(&shader),
            fragment: Some(wgpu::FragmentState {
                module: &shader,
    ...
    pub(super) fn render(
        ...
        let features: Vec<wgpu::Texture> = (0..7)
            .map(|index| self.feature_texture(device, source_extent, &format!("feature-{index}")))
            .collect();
        let feature_views: Vec<wgpu::TextureView> = features
            .iter()
        ...
        let output_texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-output"),
            size: output_extent,
            mip_level_count: 1,
            sample_count: 1,
            dimension: wgpu::TextureDimension::D2,
        ...
        self.render_conv_pass(device, encoder, source_view, &feature_views[0], 0);
        for pass_id in 1..7 {
            self.render_conv_pass(
                device,
                encoder,
                &feature_views[pass_id - 1],
                &feature_views[pass_id],
                pass_id as u32,
            );
        }
    ...
    fn feature_texture(
        ...
        device.create_texture(&wgpu::TextureDescriptor {
            label: Some(&format!("suisuiview-realtime-anime4k-m-{label_suffix}")),
            size,
            mip_level_count: 1,
            sample_count: 1,
            dimension: wgpu::TextureDimension::D2,
    ...
    fn render_conv_pass(
        ...
        let params_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-conv-params"),
            contents: bytemuck::bytes_of(&params),
            usage: wgpu::BufferUsages::UNIFORM,
        });
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-conv-bind-group"),
            layout: &self.conv_bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
        ...
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-conv-pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: output_view,
                resolve_target: None,
                ops: clear_store_ops(),
    ...
    fn render_combine_pass(
        ...
        let entries = [
            texture_binding(0, feature_views[0]),
            texture_binding(1, feature_views[1]),
            texture_binding(2, feature_views[2]),
            texture_binding(3, feature_views[3]),
            texture_binding(4, feature_views[4]),
            texture_binding(5, feature_views[5]),
            texture_binding(6, feature_views[6]),
        ];
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-combine-bind-group"),
            layout: &self.combine_bind_group_layout,
            entries: &entries,
        });

        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-combine-pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: output_view,
                resolve_target: None,
                ops: clear_store_ops(),
    ...
    fn render_final_pass(
        ...
        let entries = [
            ...
            texture_binding(1, feature_view),
        ];
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-final-bind-group"),
            layout: &self.final_bind_group_layout,
            entries: &entries,
        });

        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("suisuiview-realtime-anime4k-m-final-pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: output_view,
                resolve_target: None,
                ops: clear_store_ops(),
