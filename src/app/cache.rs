...
impl SuiSuiViewApp {
    ...
    pub(in crate::app) fn texture_cache_budget_bytes(&self) -> usize {
        texture_cache_budget_bytes_for(
            ...
            texture_cache_budget_cap_bytes(&self.settings),
        )
    }
...
pub(in crate::app) fn texture_cache_budget_bytes_for(
    ...
    let texture_page_goal = visible_page_count
        ...
        .saturating_add(1);
    // The content-derived goal keeps visible spreads bounded; the total-budget cap dominates from
    // above and MIN_TEXTURE_CACHE_BYTES floors it so the current spread always fits.
    let cap = budget_cap_bytes.max(budgets::MIN_TEXTURE_CACHE_BYTES);
    estimated_page_bytes_for_target(target_long_edge)
        .saturating_mul(texture_page_goal)
        .clamp(budgets::MIN_TEXTURE_CACHE_BYTES, cap)
}
