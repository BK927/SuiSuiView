...
impl DpiSizeGuard {
    ...
    pub(super) fn on_scale_change(&mut self, new_scale: f64, now: Instant) -> Option<(u32, u32)> {
        self.promote_confirmed_pending(now);
        self.pending_logical = None;
    ...
    pub(super) fn seed_initial(&mut self, window: &winit::window::Window) {
        ...
        let _ = self.observe_resize((size.width, size.height), Instant::now());
    }
    ...
    pub(super) fn defend_scale_change(
        ...
        match event {
            winit::event::WindowEvent::Resized(size) => {
                self.observe_resize((size.width, size.height), Instant::now())
            }
            winit::event::WindowEvent::ScaleFactorChanged {
                scale_factor,
                inner_size_writer,
            } => {
                if let Some((w, h)) = self.on_scale_change(*scale_factor, Instant::now()) {
                    let _ =
...
#[cfg(test)]
mod tests {
    use super::{DpiSizeGuard, STORM_WINDOW};
    use std::time::{Duration, Instant};
    ...
    fn stable_size_tracked_from_calm_resize() {
        ...
        assert_eq!(guard.on_scale_change(1.25, confirmed), Some((1075, 864)));
    }
    ...
    fn resize_within_storm_window_is_ignored_and_corrected() {
        ...
        guard.on_scale_change(1.25, confirmed);
        assert_eq!(
            guard.observe_resize((714, 570), confirmed + Duration::from_millis(10)),
            Some((1075, 864))
        );
        // The stable size is still the calm one: 1.5 request stays 1290x1037.
        assert_eq!(guard.on_scale_change(1.5, confirmed), Some((1290, 1037)));
    }
    ...
    fn matching_transition_resize_needs_no_correction() {
        ...
        guard.on_scale_change(1.25, confirmed);
        assert_eq!(
            guard.observe_resize((1075, 864), confirmed + Duration::from_millis(2)),
            None
        );
    }
    ...
    fn phantom_post_storm_resize_is_corrected() {
        ...
        let _ = guard.observe_resize((1920, 1230), t0);
        let confirmed = t0 + CONFIRM_WINDOW;
        guard.on_scale_change(1.25, confirmed + Duration::from_millis(1));
        guard.on_scale_change(1.5, confirmed + Duration::from_millis(3));
        assert_eq!(
            guard.observe_resize((2308, 1487), confirmed + Duration::from_millis(15)),
            Some((1920, 1230))
        );
        assert_eq!(
            guard.observe_resize((1920, 1230), confirmed + Duration::from_millis(30)),
            None
        );
    }
    ...
    fn late_phantom_right_before_scale_change_cannot_poison() {
        ...
        let mut guard = DpiSizeGuard::new();
        guard.observe_scale(1.5);
        let _ = guard.observe_resize((1290, 840), t0); // logical 860x560
        let calm = t0 + CONFIRM_WINDOW + Duration::from_secs(2);
        let _ = guard.observe_resize((1434, 1202), calm); // the phantom
        assert_eq!(
            guard.on_scale_change(1.25, calm + Duration::from_micros(400)),
    ...
    fn genuine_calm_resize_confirms_and_becomes_stable() {
        ...
        let mut guard = DpiSizeGuard::new();
        guard.observe_scale(1.5);
        let _ = guard.observe_resize((1290, 840), t0);
        let calm = t0 + CONFIRM_WINDOW;
        let _ = guard.observe_resize((1520, 840), calm);
        assert_eq!(
            guard.on_scale_change(1.25, calm + CONFIRM_WINDOW),
            Some((1267, 700))
        );
    }
    ...
    fn storm_sequence_returns_same_size_no_drift() {
        ...
        let first = guard.on_scale_change(1.25, confirmed + Duration::from_millis(1));
        // winit hands us a corrupted in-flight size mid-storm.
        let _ = guard.observe_resize((714, 570), confirmed + Duration::from_millis(3));
        let second = guard.on_scale_change(1.5, confirmed + Duration::from_millis(6));
        // Another corrupt resize (the grow direction).
        let _ = guard.observe_resize((1438, 1211), confirmed + Duration::from_millis(12));
        let third = guard.on_scale_change(1.25, confirmed + Duration::from_millis(18));
    ...
    fn zero_size_is_ignored() {
        ...
        let mut guard = DpiSizeGuard::new();
        guard.observe_scale(1.5);
        let _ = guard.observe_resize((0, 0), now);
        // No stable size recorded, so a scale change has nothing to assert.
        assert_eq!(guard.on_scale_change(1.25, now + CONFIRM_WINDOW), None);
    }
    ...
    fn unknown_scale_start_returns_none_until_seeded() {
        ...
        let _ = guard.observe_resize((1290, 1037), now);
        assert_eq!(guard.on_scale_change(1.25, now), None);
        // Once seeded and given a calm, confirmed resize, it defends the size.
        guard.observe_scale(1.5);
        let _ = guard.observe_resize((1290, 1037), now + STORM_WINDOW);
        assert_eq!(
            guard.on_scale_change(1.25, now + STORM_WINDOW + CONFIRM_WINDOW),
            Some((1075, 864))
        );
    }
}
